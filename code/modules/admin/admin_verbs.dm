	...
	if(src.holder && (src.holder.rights & R_ADMIN))
		...
		if (is_donator(src))
			if (get_don_tier(src) >= 3)
				...
				src << "Only Tier 3 or higher donators can use this command."
		else
			src << "Only tier 3 or higher donators and administrators can use this command."
